...
interface ProductCellActionProps {
  item: ProductColumnProps;
}

const ProductCellAction: FC<ProductCellActionProps> = ({ item }) => {
  ...
  const onUpdate = () => {
    router.push("/" + params.storeId + "/products/" + item.id);
  };
  ...
  const onDelete = async () => {
    try {
      ...
      const res = await axios.delete(
        "/api/" + params.storeId + "/products/" + item.id
      );
  ...
  return (
    <>
      <DeleteModal
        title={"Delete" + " " + item.name.toLowerCase() + " " + "product"}
